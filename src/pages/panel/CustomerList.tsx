...
import { useCustomers, useUpdateCustomerById, useDeleteCustomer } from "../../hocks/useCustomers";
		...
		<div className="p-6 space-y-4">
			...
			{!isLoading && !isError && filtered.length > 0 && (
				<table className="w-full text-sm">
					...
					<tbody>
						{pageRows.map((c) => {
							if (confirmDeleteId === c.idCustomer) {
								return (
									<tr key={c.idCustomer} className="border-b border-white/5">
										<td colSpan={6} className="py-2">
											<ConfirmDelete
												message={`Видалити клієнта "${c.nameCustomer}"?${c.storesCount ? " Спершу видаліть/перепризначте його магазини." : ""}`}
												pending={deleteCustomer.isPending && deleteCustomer.variables === c.idCustomer}
												onCancel={() => setConfirmDeleteId(null)}
												onConfirm={() => deleteCustomer.mutate(c.idCustomer, { onSuccess: () => setConfirmDeleteId(null) })}
											/>
										</td>
									</tr>
								);
							...
							return (
								<tr key={c.idCustomer} className="border-b border-white/5 hover:bg-white/5">
									...
									<td className="py-2 text-right">
										<Button type="button" variant="ghost" size="sm" onClick={() => setConfirmDeleteId(c.idCustomer)}>
											🗑
										</Button>
									</td>
								</tr>
			...
			{deleteCustomer.isError && (
				<ErrorBanner message={`Не вдалось видалити клієнта: ${(deleteCustomer.error as Error).message}`} />
			)}
		</div>
	);
}
