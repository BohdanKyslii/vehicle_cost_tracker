...
| Живе тестування деструктивних ендпоінтів (`import_file` тощо) | Локальний бекенд-`.env` (`vehicle_tracker_api`) конфігурований на PostgreSQL ПРЯМО НА ПРОДІ (`DB_HOST=192.168.0.114`, Pi), не на локальній dev-БД — DELETE+INSERT-логіка (напр. "перезаливка за датами" в `import_waybills`) при тестовому запуску мутує реальні прод-дані. Не запускай такі ендпоінти "щоб перевірити" без явного дозволу користувача (Фаза 22, 2026-09-06) |
