...
За `CODING_GUIDE.md`: **Фази 1-22 набрані в код і задеплоєні** (Фаза 22
змержена в `main`, PR #19), плюс одна фіча поза нумерацією гайду
(`panel-management`, [[decisions.md]]). **Фаза 23** (служби доставки,
`/carriers`) реалізована 2026-09-06 в окремій гілці `faza-23-carriers`,
ще НЕ змержена — `npm run build`+`eslint` чисті, живий тест (Крок 23.10)
НЕ проведено. **Не перевірено цією сесією:** чи запушений бекенд
`vehicle_tracker_api` для Фаз 18-19 (`apps.logistics`) моделі/міграції —
серіалізатори/views/urls для `apps.logistics` звірено читанням і
виявились готовими (використані Фазою 23); якщо саме моделі/міграції
Фаз 18-19 досі не запушені окремо, прод може працювати проти старішого
бекенду, ніж очікує фронтенд.
...
- **Фаза 22 (2026-09-06)** — імпорт накладних з 1С, `/waybills/import`
  (`WaybillImportForm.tsx`, `apiFetchMultipart`). Гілка
  `faza-22-1c-import`, пізніше змержена в `main` (PR #19). Живий тест
  реальним файлом свідомо не проведений — ризик для прод-БД
  ([[decisions.md]]).
- **Фаза 23 (2026-09-06, наступна сесія того самого дня)** — служби
  доставки, `/carriers`: `CarrierShipmentList`/`Form.tsx` (локед-режим +
  сканування QR, той самий флоу, що `HiredTripForm`), `CarrierCostImport.tsx`
  (CSV-реєстр витрат). Бекенд (`apps/logistics`) уже був готовий і
  задеплований разом з Фазою 18 — звірений напряму з кодом. Гілка
  `faza-23-carriers`, `npm run build`+`eslint` чисті, живий тест
  (Крок 23.10) НЕ проведений з тієї самої причини, що Фаза 22.

## Наступні кроки

1. `WaybillDetail`, `/waybills/unassigned`/`returns` — лишаються
   `PlaceholderPage` (не в обсязі Фази 22).
2. Змерджити `faza-23-carriers` в `main` — перед цим Крок 23.10 вручну
   (живий тест) і звірка мапінгу `parseCarrierCostsCsv.ts` з реальним
   файлом-реєстром служби доставки.
3. Аналітика/графіки (Recharts) — і бекенд (`apps.analytics`), і
   фронтенд свідомо не почато.
4. Перевірити backend-репо `vehicle_tracker_api`: чи запушено моделі/
   міграції `apps.logistics` для Фаз 18-19 окремо від
   серіалізаторів/views (які вже підтверджено готовими для Фази 23).
