...
>
> **2026-09-17 — попередній запис сам виявився хибним:** `import_file`
> РЕАЛЬНО існує й працює, підтверджено живим імпортом на прод (ESP/OPT
> xls, 2613+77 рядків, 0 помилок). Того ж дня знайдено й виправлено
> ОКРЕМУ, справжню проблему в реєстрі накладних `/waybills` (не в самій
> формі імпорту): `src/api/waybills.ts` бив у неіснуючий шлях
> `/api/waybills/...` замість `/api/waybill-records/...`, і навіть
> після виправлення шляху сторінка падала — бекенд віддає сирий
> DRF-пейдж по товарних рядках (snake_case, без агрегації по накладній,
> без поля `status`), а фронтенд чекав агрегований camelCase
> `WaybillSummary[]`. Обидва виправлено й задеплоєно; агрегація по
> `waybill_number` — досі НЕ зроблена (кожен товарний рядок = окремий
> рядок таблиці). Деталі — `tasks.md`, [[waybills-list-real-api-gap]]
> (пам'ять Claude Code).
...
2026-09-17: `import_file` (`apps/waybills/views.py` + `importers/`
(`rubin_csv.py`, `esp_opt_xls.py`) + `importing.py`) **існує, написаний
і реально працює** — підтверджено живим запитом на прод (реальний
імпорт ESP/OPT xls-реєстрів, 2613+77 рядків, 0 помилок) і раніше вже
Rubin CSV (1828/1828, [[rubin-csv-blank-customer-id-bug]]). Те, що
виглядало як "імпорт зламаний", насправді було ОКРЕМИМ багом на
фронтенді, не пов'язаним з існуванням ендпоінту: `src/api/waybills.ts`
(список/деталі/канал/unassigned — НЕ сам імпорт, той завжди був у
`waybillImport.ts` і працював правильно) бив у неіснуючий
`/api/waybills/...` замість реального `/api/waybill-records/...`, і
навіть після виправлення шляху малював порожню/падаючу сторінку, бо
