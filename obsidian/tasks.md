...
> довідників. Цієї ж сесії реалізовано **Фазу 22** (див. нижче) в
> окремій гілці `faza-22-1c-import`, ще не змерджена. Детальніший
> наратив — `STATE.md`, хронологія — `CHANGES.md`.
...
1. Змерджити `faza-22-1c-import` в `main` — але перед цим виконати
   Крок 22.8 вручну (живий тест реальним файлом), бо ця сесія його
   свідомо пропустила (ризик для прод-БД). Після мержу — `WaybillDetail`,
   `/waybills/unassigned`, `/waybills/returns` лишаються не набраними.
...
4. Перевірити backend-репо `vehicle_tracker_api` — попередні нотатки
   ...
   бекенду не перевіряла (окрім `apps/waybills`, який звірено для
   Фази 22 і виявився готовим). Якщо `apps.logistics` досі не
   запушено — Фази 18-21 і panel-management у проді можуть працювати
   проти старішого бекенду, ніж очікується.
