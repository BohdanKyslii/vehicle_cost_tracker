...
1. **134 підозрілих тестових товари в проді** — знайдено одноразовим
   аудитом (видалений після), не видалені (не було явного дозволу
   користувача). Деталі — [[open-items-2026-09-10]].
2. **`CATEGORY_DEFAULTS.CHILD_OTHER` (id 15) застарілий** — тепер
...
3. `/waybills/unassigned`, `/waybills/returns` лишаються
   `PlaceholderPage` (`WaybillDetail` вже НЕ в цьому списку —
   реалізовано 2026-09-17, див. вище).
4. Аналітика/графіки (Recharts) — і бекенд (`apps.analytics`), і
   фронтенд свідомо не почато.
5. Перевірити backend-репо `vehicle_tracker_api` — попередні нотатки
...
6. Дрібне: `findEventGroup()` тепер групує за явним маркером
