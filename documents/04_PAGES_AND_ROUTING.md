...
| Назва | `car.nameCar` |
| Статус | ✅ 2026-08-28 — інтерактивний `<select>` (не просто бейдж): логіст міняє статус прямо зі списку, без відкриття `CarForm`. Викликає окремий бекенд-ендпоінт `POST /cars/{id}/change_status/` (`useChangeCarStatus`), що пише запис в `CarStatusLog` — на відміну від збереження статусу через звичайну форму (`updateCar`, PATCH), яке історію НЕ веде |
...
⚠️ Другий бекенд-баг, той самий день, серйозніший: авто **З** причепом
завжди падало з 500 (`IntegrityError` — орфанна колонка `model` в
таблиці `trailers`, якої вже немає в Django-моделі), а через
відсутність `transaction.atomic()` в `CarSerializer.create()` авто все
одно напівстворювалось у БД без причепа. Наступна спроба з тим самим
номером після цього падала вже на СПРАВЖНІ 400 "вже існує" — саме тому
баг виглядав нерозв'язним при повторних спробах. Виправлено міграцією
`0004_fix_trailer_schema_drift` + атомарністю (backend commit `deb38c8`).
Заразом видалено з фронтенду поле "Модель причепа" (`Trailer.model`) —
воно ніколи не мапилось на бекенд (DRF мовчки ігнорував), тобто
введене значення завжди губилось безслідно.

✅ Резинхронізовано 2026-08-28 (той самий день, друга ітерація):
- Текстові/числові поля й чекбокси в `CarForm` тепер заблоковані
  (`disabled`) за замовчуванням для ІСНУЮЧОГО авто (`isEdit=true`) —
  ці дані змінюються рідко, легко натиснути повз. Кнопка
  "✏️ Редагувати" (`isEditingDetails` стан) розблоковує їх. Три
  `<select>` (режим/статус/водій) лишаються завжди активними — не
  потребують розблокування для швидких змін. Для НОВОГО авто
  (`isEdit=false`) блокування взагалі нема, все одразу редаговане.
- `CarStatus` (і сам select статусу, і бейдж) отримав два нових
  значення: `pause` ("Пауза" — вимушений простій через регуляцію часу
  праці водія/авто, тахограф) і `driver_downtime` ("Простій (водій)" —
  лікарняний/відпустка/сімейні обставини, без деталізації причини на
  рівні статусу самого; причина фіксується окремо через `reason` у
  `change_status`).
