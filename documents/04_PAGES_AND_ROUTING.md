...
| `delivery` (full) | Одометр + Кількість палет на точку + скан **основної** накладної + кнопка "📷 Ще одна накладна цієї точки" (0-3 додаткові, кожна — окремий запис `delivery` без одометра/палет, щоб не задвоїти суму по точці) |
| `delivery` (daily) | Лише скан накладної, **без** одометра (одометр разово вносить `depot_start`) — підпис тайла "Скан накладної" замість "Вивантаження" (`eventTypeLabel(type, mode)`) |
...
✅ Резинхронізовано 2026-08-27 — QR-сканування накладних **реалізоване**
(componenet `QRScanner.tsx`, Фаза 15): для `delivery` камера відкривається
одразу при вході на екран (`scannerOpen` ініціалізується `true`, коли
`requiresWaybill(type)`), форма з підтягнутим номером/датою з'являється
лише після успішного скану. Повторне сканування/збереження вже
відсканованої сьогодні накладної блокується (`isAlreadyScannedToday` в
`EventForm.tsx`, звіряється з `useTodayEvents`) — камера лишається
відкритою з попередженням, доки не відскановано іншу накладну; кнопка
"📷 Сканувати QR" ховається одразу після успішного скану.
