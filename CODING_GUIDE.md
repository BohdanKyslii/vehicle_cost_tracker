...
>
> **Оновлено 2026-09-06:** **Фаза 22** (імпорт накладних з 1С) —
> фронтенд-частина набрана в коді (гілка `faza-22-1c-import`, звірена
> напряму проти реального `apps/waybills/views.py` й `importing.py` в
> `vehicle_tracker_api` — форма відповіді й права `IsManagerOrHeadOnly`
> збіглись з припущенням плану один-в-один). `npm run build` (`tsc -b`
> + `vite build`) проходить чисто. **Живий тест реальним файлом
> (Крок 22.8) НЕ проведено** — локальний бекенд у цьому середовищі
> конфігурований на PostgreSQL прямо на проді (Pi, `192.168.0.114`),
> а `import_file` робить "перезаливку за датами" (DELETE+INSERT) і
> може завести нових Customer/Store через `get_or_create` при
> невдалому тестовому запуску — ризик для прод-даних визнано
> невиправданим без окремого dev-стенду/бекапу. Перевір Крок 22.8
> вручну перед тим, як вважати фазу готовою до реального використання.
>
> **Оновлено 2026-09-06 (та сама сесія):** **Фаза 23** (служби
> доставки) — фронтенд-частина набрана в коді (гілка
> `faza-23-carriers`, тіло Кроків 23.2-23.9 набрано практично один-в-
> один з текстом гайду вище). `npm run build` (`tsc -b` + `vite build`)
> і `eslint` чисті. **Живий тест (Крок 23.10) НЕ проведено** — той
> самий ризик для прод-БД, що й у Фазі 22 (див. вище й `decisions.md`):
> локальний бекенд б'є в прод-PostgreSQL, а тут це ще й новий,
> раніше не викликаний ендпоінт (`CarrierCostViewSet.perform_create`).
> Парсер `parseCarrierCostsCsv.ts` лишається **тимчасовим** — мапінг
> колонок CSV не звірявся з реальним файлом-реєстром жодної служби
> доставки (Крок 23.6 вище). Перед мержем у `main`: живий тест
> Кроку 23.10 і звірка мапінгу CSV з реальним файлом від Нової
> Пошти/Міст Експрес.

> **Оновлено 2026-09-10:** **Довідники товарів/клієнтів/магазинів/категорій**
> (`/panel/products`, `/panel/categories`, `/panel/customers`, `/panel/stores`)
> — цей шматок УЗАГАЛІ не описаний вище як окремий "Крок"/"Фаза": набраний
> і задеплоєний поза цим гайдом, документується тут постфактум.
> - Кожен довідник: список (клік по заголовку колонки — інлайн фільтр,
>   текстовий або `<select>`, компонент `FilterableHeader.tsx`),
>   клієнтська пагінація 25/50/100 (`usePagination.ts` + `PageSizeSelect.tsx`
>   + вже наявний `Pagination.tsx` з Кроку 8.7, тепер не лише на
>   Накладних), інлайн-перемикач статусу прямо в рядку (той самий
>   принцип, що статус авто в Кроці 16.4), і 🗑 з інлайн-підтвердженням
>   (`ConfirmDelete`, той самий компонент, що на подіях водіїв) — усе
>   чотири довідники однаковим паттерном.
> - **Категорії товарів** — новий довідник, якого не було: список,
>   картка (локед-режим редагування, як CarForm), масовий імпорт з Excel
>   (`CategoryImport.tsx`, `/panel/categories/import`). Товар-категорія —
>   дерево (батьківська категорія), ProductCategory.id — автоінкремент
>   на бекенді (НЕ 1С-ID, на відміну від товару/клієнта/магазину), тож
>   імпорт зіставляє батько/дитина за НАЗВОЮ (унікальне поле), а не за
>   ID з файлу.
> - **Системний баг пагінації, виправлено скрізь:** DRF пагінує всі
>   list-ендпоінти по `PAGE_SIZE=10` (`config/settings.py` бекенду);
>   кожен `fetch*` у `src/api/*.ts` раніше читав лише `data.results` з
>   першої сторінки, мовчки губив решту при >10 записах. Виправлено
>   спільним `fetchAllPages<T>()` (`src/api/config.ts`), який іде по
>   `data.next`, доки не `null` — усі list-фетчери тепер ідуть через
>   нього. Дорогою впіймано й виправлено супутній баг: `data.next` від
>   DRF — абсолютний URL, який уже містить шлях `API_BASE` (напр.
>   `https://warehouse.mom/api`), тож наївна конкатенація давала
>   `/api/api/...` і 404 — тепер префікс `API_BASE` явно віднімається.
> - **Системний баг порожніх карток редагування, виправлено в 9 формах:**
>   `ProductForm`, `CustomerForm`, `StoreForm`, `CarForm`, `DriverForm`,
>   `CarrierShipmentForm`, `HiredTripForm`, `MonthlyCostsForm`, нова
>   `CategoryForm` — усі ініціалізували стан через
>   `useState(existing?.field ?? default)`, а `existing` (React Query)
>   на холодному кеші ще `undefined` у момент першого рендеру, тож форма
>   лишалась порожньою НАЗАВЖДИ (не пересинхронізувалась після
>   довантаження). Виправлено патерном "adjust state during render"
>   (React docs) — НЕ `useEffect` (в репо є eslint-правило
>   `react-hooks/set-state-in-effect`, яке саме це забороняє).
> - **PWA:** `manifest.icons` був порожній — "Додати на головний екран"
>   давав іконку-заглушку; додано `public/pwa-icon-512.png` (лого
>   RUBIN, 192/512, без `purpose:"maskable"` — лого без safe-zone,
>   обрізалось б). `workbox.maximumFileSizeToCacheInBytes` піднято до
>   4 МіБ (дефолтні 2 МіБ вже блокували сам `npm run build`, бандл
>   перевищив ліміт).
> - **Реальні дані залиті в прод:** Категорії (85: 9 коренів + 76
>   дітей), Клієнти (382), Магазини (2152), Товари (956 з
>   `шаблон-товари.xlsx` + 164 з реєстру переміщень на АЗС). До цього
>   ~1088 товарів уже існували з побитими категоріями (наслідок більш
>   раннього сирого імпорту, де числовий "ID категорії" з файлу
>   випадково збігався з реальними ID зовсім інших категорій) —
>   виправлено одноразовим інструментом (видалений після використання).
> - **⚠️ Спростування Кроку 22 вище:** нотатка від 2026-09-06 каже, що
>   `import_file` "звірено напряму проти реального
>   `apps/waybills/views.py`" — перевірено ще раз 2026-09-10 напряму
>   проти `vehicle_tracker_api/apps/waybills/views.py`: **цього
>   ендпоінта на бекенді НЕМАЄ** (лише стандартний CRUD +
>   `unassigned`/`assign_channel`). Вкладка "Імпорт із 1С" зараз падає
>   404 для БУДЬ-ЯКОЇ юрособи (Рубін/ЄСП/ОПТ) — це бекенд-задача, не
>   зроблена.
